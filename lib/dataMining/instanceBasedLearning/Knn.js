...
function Knn() {
  ...
  this.defaultParams = {
    ...
    predict: {
      formula: function (distances, data, attributeIndex, maxDistance) {
        var result,
            resultArray = distances.map(function (distance, index) {
              var normalizedDistance = (maxDistance === 0 ? 0 : (distance / maxDistance));

              return data[index][attributeIndex] * (1 - normalizedDistance);
            });

        result = resultArray.reduce(function (prevValue, value) {
...
Knn.prototype.setConfig = function configFunc(params) {
  ...
  if (!_.isFunction(this.params.predict.formula)) {
    throw new Error("'formula' property of 'predict' needs to be a function");
  }
...
Knn.prototype.execute = function executeFunc(data) {
  ...
  kDistances = indexesDistances.map(function (index)  {
    return distances[index];
  });

  kData = indexesDistances.map(function (index) {
    return data[index];
  });

  that.params.predict.attributes.forEach(function (attrIndex) {
    result[attrIndex] = that.params.predict.formula(kDistances, kData, attrIndex, maxDist);
  });
